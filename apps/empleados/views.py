...
def renuncia_empleado(request, pk):
    ...
    return render(request, 'empleados/renuncia.html', {
        'empleado': empleado,
    })
...
def recontratar_empleado(request, pk):
    ...
    return render(request, 'empleados/recontratar.html', {
        'empleado': empleado,
    })
            ...
            try:
                ...
                if created:
                    ...
                    if not empleado.user:
                        ...
                        UserModel = get_user_model()
                        temp_password = generar_password()
                        username = id_orig.lower()
                        if UserModel.objects.filter(username=username).exists():
                            username = f'{username}_{empleado.id}'
                        user = UserModel.objects.create_user(
                            username=username, password=temp_password,
                            email=empleado.email, first_name=nombre,
