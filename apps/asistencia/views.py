...
def api_today(request):
    ...
    return JsonResponse({
        'registros': [{
            ...
            'fuente': r.fuente,
            'ip': r.ip_address or '',
            'ubicacion_lat': float(r.ubicacion_lat) if r.ubicacion_lat else None,
            'ubicacion_lng': float(r.ubicacion_lng) if r.ubicacion_lng else None,
            'ubicacion_direccion': r.ubicacion_direccion or '',
        } for r in recientes],
